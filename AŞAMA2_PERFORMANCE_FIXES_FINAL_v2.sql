-- ========================================
-- CHIZO DATABASE - AŞAMA 2: PERFORMANS İYİLEŞTİRMELERİ (FINAL v2)
-- ========================================
-- Tarih: 2025-10-21
-- Süre: ~2 saat
-- Kritiklik: 🟡 ORTA
...
-- ===========================================
-- 3. user_photos.photo_order NULL FIX (AKILLI STRATEJI)
-- ===========================================
...
BEGIN
  RAISE NOTICE 'Fixing NULL photo_order values...';

  -- Her kullanıcı için NULL photo_order'ları düzelt
  FOR rec IN
    SELECT DISTINCT user_id
    FROM user_photos
    WHERE photo_order IS NULL
  LOOP
    -- Bu kullanıcının en büyük photo_order değerini bul
    SELECT COALESCE(MAX(photo_order), 0) + 1
    INTO v_next_order
    FROM user_photos
    WHERE user_id = rec.user_id AND photo_order IS NOT NULL;

    -- NULL olan kayıtları sırayla numaralandır
    WITH numbered AS (
      SELECT id, ROW_NUMBER() OVER (ORDER BY created_at) AS rn
      FROM user_photos
      WHERE user_id = rec.user_id AND photo_order IS NULL
    )
    UPDATE user_photos up
    SET photo_order = v_next_order + n.rn - 1
    FROM numbered n
    WHERE up.id = n.id;

    GET DIAGNOSTICS v_fixed_count = ROW_COUNT;
    RAISE NOTICE 'Fixed % NULL photo_order for user %', v_fixed_count, rec.user_id;
  END LOOP;
...
-- AŞAMA 2 TAMAMLANDI ✅
-- ========================================
-- Sonraki adım: DART_CODE_CHANGES.md (kod güncellemeleri)
-- veya AŞAMA3_OPTIMIZATION_FIXES.sql (opsiyonel)
-- ========================================
