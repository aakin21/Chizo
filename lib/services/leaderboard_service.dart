...
class LeaderboardService {
  ...
  static Future<List<UserModel>> getTopWinners({int limit = 10}) async {
    try {
      ...
      // debugPrint('📊 Query response count: ${(response as List).length}');
      
      final users = (response as List)
          .map((json) {
            // debugPrint('👤 User: ${json['username']}, wins: ${json['wins']}, photos: ${json['user_photos']?.length ?? 0}');
      ...
      // debugPrint('Error getting top winners: $e');
      return [];
    }
  }
  ...
  static Future<List<UserModel>> getTopWinRate({int limit = 10}) async {
    try {
      ...
      // debugPrint('❌ Error getting top win rate: $e');
      return [];
    }
  }
